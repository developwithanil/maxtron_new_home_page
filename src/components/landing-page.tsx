...
function HeroVisual({ reduce }: { reduce: boolean }) {
  ...
  const sparklines = [
    "M0,12 Q5,2 10,8 T20,3 T30,9 T40,2",
    "M0,15 L8,15 L16,8 L24,12 L32,3 L40,1",
    "M0,5 Q8,15 16,7 T32,13 T40,3"
  ];
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          background: `radial-gradient(circle at 20% 20%, #7c3aed 0%, transparent 50%), radial-gradient(circle at 80% 80%, #2563eb 0%, transparent 50%)`
        }}
      ...
      />
      <motion.div
        className="relative overflow-hidden rounded-2xl border border-violet-200/50 bg-white/80 p-2 shadow-[var(--hc-shadow-lg)] backdrop-blur-md"
        whileHover={reduce ? undefined : { scale: 1.01, rotate: 0.2 }}
        transition={springSoft}
      >
        <motion.div
          className="relative overflow-hidden rounded-xl bg-gradient-to-br from-[#0c0a21] via-[#141133] to-[#070514] p-6 border border-white/10"
        ...
        >
          ...
          <motion.div
            className="pointer-events-none absolute inset-x-0 top-0 h-[2px] bg-gradient-to-r from-transparent via-violet-400 to-transparent opacity-80"
          ...
          >
            <div>
              <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.25em] text-violet-400">
                // COMMAND CENTER
              </p>
              <p className="mt-0.5 text-base font-extrabold tracking-wide text-white">White-Label Solutions</p>
            </div>
          ...
          <motion.div className="grid grid-cols-2 gap-3">
            {maxtronProducts.slice(0, 4).map((p, i) => (
              <motion.a
                ...
                className="group relative overflow-hidden rounded-xl border border-white/10 bg-white/5 p-4 transition-all hover:border-violet-400/50 hover:bg-white/10"
              ...
              >
                ...
                <div className="absolute -right-8 -top-8 h-16 w-16 rounded-full bg-violet-500/10 blur-xl opacity-0 group-hover:opacity-100 transition-opacity" />

                <p className="text-[12px] font-bold leading-snug text-white sm:text-xs">{p.title}</p>
                <p className="mt-2 flex items-center gap-1 text-[10px] font-semibold text-violet-300/90 transition-colors group-hover:text-violet-200">
                  Launch Platform
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" aria-hidden className="transition-transform group-hover:translate-x-0.5">
          ...
          <motion.div
            className="relative mt-6 border-t border-white/10 pt-6"
          ...
          >
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <motion.div
                  key={m.label}
                  className="group/stat rounded-xl border border-white/5 bg-white/5 px-2.5 py-3 text-center transition-colors hover:bg-white/10 hover:border-white/10"
                  initial={reduce ? false : { opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.6 + i * 0.08, duration: 0.45, ease: easeOut }}
                  whileHover={reduce ? undefined : { scale: 1.03 }}
                >
                  <p className="text-base font-extrabold text-white tracking-tight">{m.pct}</p>
                  <p className="text-[9px] font-semibold text-violet-300 mt-0.5">{m.label}</p>


                  <div className="mt-2 flex justify-center h-4 items-center">
                    <svg width="40" height="16" viewBox="0 0 40 16" fill="none" className="stroke-violet-400/60 group-hover/stat:stroke-violet-300 transition-colors">
                      <path d={sparklines[i]} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </div>
                </motion.div>
              ))}
  ...
  return (
    <Reveal amount={0.3}>
      <motion.div className="hc-marquee-wrap relative overflow-hidden border-y border-violet-100/80 bg-violet-50/50 py-5">
        <motion.div
          className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 bg-gradient-to-r from-violet-50/90 to-transparent"
          aria-hidden
        />
        <motion.div
          className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-gradient-to-l from-violet-50/90 to-transparent"
          aria-hidden
        />
      ...
      <main className="overflow-x-clip">
        <section className="hc-hero-mesh relative overflow-hidden px-5 pb-8 pt-8 sm:px-8 sm:pb-12 sm:pt-10 lg:px-10 lg:pt-12">
          ...
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
            <motion.div
              ...
              style={{ backgroundImage: "url('/tech_background.png')" }}
            ...
            <div className="hc-dot-grid absolute inset-0 opacity-40" aria-hidden />
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-2 lg:gap-20">
            <div>
              ...
              >
                <SectionBadge>Maxtron.ai · White-Label Solutions</SectionBadge>
              </motion.div>
              ...
              >
                <span className="hc-gradient-text hc-gradient-text-animate">White-Label Solutions</span>
                <br />
          ...
          <div className="mx-auto max-w-7xl">
            <Reveal>
              <div className="mx-auto max-w-2xl text-center">
                <SectionBadge>Why White-Label Solutions</SectionBadge>
        ...
        <section id="about" className="border-t border-violet-50 bg-violet-50/30 px-5 py-20 sm:px-8 sm:py-28 lg:px-10">
          <div className="mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-2 lg:gap-20">
            <Reveal>
              <div className="relative aspect-[4/3] overflow-hidden rounded-3xl shadow-[var(--hc-shadow-lg)] border border-violet-100 bg-gradient-to-br from-[#1e1b4b] via-violet-950 to-indigo-950">

                <div className="absolute top-0 inset-x-0 h-10 bg-black/25 backdrop-blur-md border-b border-white/10 px-4 flex items-center gap-2 z-20">
                  <div className="h-2.5 w-2.5 rounded-full bg-[#ef4444]" />
                  <div className="h-2.5 w-2.5 rounded-full bg-[#eab308]" />
                  <div className="h-2.5 w-2.5 rounded-full bg-[#22c55e]" />
                  <div className="mx-auto bg-white/10 rounded-md px-4 py-0.5 text-[9px] font-mono text-white/60 tracking-wider">
                    maxtron.ai/dashboard
                  </div>
                </div>
                ...
                <div className="absolute inset-0 pt-10">
                  <img
                    ...
                    alt="Maxtron Platform UI Screens"
                    className="w-full h-full object-cover object-top opacity-85 transition-transform duration-700 hover:scale-105"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0f0c24] via-transparent to-[#0f0c24]/30 pointer-events-none" />
                </div>
                ...
                <div className="absolute bottom-6 left-6 right-6 z-10 grid grid-cols-2 gap-3">
                  {[
                    ...
                    <motion.div
                      key={item.label}
                      className="rounded-xl border border-white/20 bg-black/45 p-3.5 backdrop-blur-md"
                      initial={reduce ? false : { opacity: 0, y: 12, scale: 0.96 }}
                      whileInView={{ opacity: 1, y: 0, scale: 1 }}
                      viewport={{ once: true }}
                      transition={{ delay: 0.2 + i * 0.08, duration: 0.45, ease: easeOutExpo }}
                      whileHover={reduce ? undefined : { scale: 1.03, backgroundColor: "rgba(0,0,0,0.6)" }}
                    >
                      <p className="text-sm font-bold text-white">{item.label}</p>
                      <p className="mt-0.5 text-[10px] text-violet-300">{item.sub}</p>
                    </motion.div>
                  ))}
                </div>
              </div>
            ...
            <Reveal delay={0.1}>
              ...
              <p className="mt-4 text-base leading-relaxed text-gray-600">
                White-Label Solutions are Maxtron&apos;s curated showcase. Every product in one polished experience, built
            ...
            <StaggerReveal className="grid gap-12 sm:grid-cols-2 lg:grid-cols-4">
              <StaggerItem>
                <div className="flex items-center gap-2.5">
                  <MaxtronLogo size={32} />
                  <span className="text-lg font-bold text-gray-900">White-Label Solutions</span>
                </div>
                <p className="mt-4 text-sm leading-relaxed text-gray-600">
                  Maxtron&apos;s white-label solutions hub for AI, Web3, mobile, trading, and RWA tokenization in one
