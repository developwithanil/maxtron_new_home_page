...
const secondrow=[
    "Commission Rate","Upto 20% per deal","Upto 25% per deal","Upto 30% per deal"
]
const thirdrow=[
    "Monthly Referrals","1–4","5–9","10+"
]
const fourthrow=[
    {points:["Bonus Perks"]},
    {points:["Access to affiliate toolkit"]},
    {points:["Co-branded content","Optimization tips"]},
    {points:["Dedicated manager","Exclusive bonuses","Early access to new products"]},
]
...
<div className="hidden lg:flex flex-row gap-5 mt-16">
  {firstrow.map((item, index) => (
    <div
      key={index}
      className={`flex justify-center items-center gap-2.5 flex-[1_0_0] p-4 rounded-xl ${index === 0 ? "opacity-0" : "opacity-100"}`}
...
<div className="hidden lg:flex flex-row gap-5 mt-7">
  {secondrow.map((item, index) => (
    ...
    >
      <p className={`text-[#2A2A2A] text-center font-[Switzer] text-2xl ${index === 0 ? "font-semibold" : "font-normal"}`}>
        {item}
      </p>
    </div>
  ))}
