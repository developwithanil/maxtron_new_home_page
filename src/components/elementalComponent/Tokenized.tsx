  ...
  return (
    <div className='w-full max-w-[96rem] mx-auto flex flex-col gap-5 md:gap-9 lg:gap-12 px-4 sm:px-6 lg:px-10 py-5 md:py-7 lg:py-10'>
      ...
      <p className='w-[90%] md:w-[75%] lg:w-[60%] mx-auto text-center text-[#2A2A2A] font-[Switzer] text-3xl lg:text-4xl xl:text-5xl mb-5 font-black leading-[100%]  lg:leading-[160%] tracking-tight'>
        We tokenized and launched over <span className="text-[#7A35C1]">15+</span> assets
      </p>
      <p className='w-[80%] md:w-[65%] lg:w-[55%] mx-auto shrink-0 self-stretch text-[#2A2A2A] text-center font-openSansHebrew text-lg font-normal leading-[20px] lg:leading-[26px]  tracking-[-0.36px]'>For founders to reimagine finance and ownership using blockchain technologies.</p>
        </div>
  ...
  {cards.map((card, idx) => (
    ...
    >
      ...
      <div className="relative z-10 p-5 flex flex-col gap-2">
        <span className="inline-block bg-[#FFF] text-[#2A2A2A] text-xs font-semibold rounded-full px-4 py-1 mb-2 w-fit">{card.title}</span>
        <p className='text-[#FFF] font-openSansHebrew text-base font-normal leading-6 tracking-[-0.16px]'>{card.desc}</p>
      </div>
    </div>
  ))}
</div>
