...
import { whylogo1, whylogo2, whylogo3, whylogo4, whylogo5, whylogo6, } from "../VectorImage";
const items = [
    {
        icon: whylogo1,
...
const WhyTokenize = () => {
    return (_jsxs("div", { className: "w-full max-w-[96rem] mx-auto flex flex-col gap-5 md:gap-9 lg:gap-12 px-4 sm:px-6 lg:px-10 py-5 md:py-7 lg:py-10", children: [_jsx("p", { className: "w-full text-center text-[#2A2A2A] font-[Switzer] text-3xl lg:text-4xl xl:text-5xl mb-5 font-black leading-[160%] tracking-tight", children: "Why tokenize?" }), _jsx("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5 lg:gap-8", children: items.map((item, index) => (_jsxs("div", { className: "flex flex-col items-start gap-2 md:gap-3 lg:gap-4", children: [item.icon(), _jsx("h3", { className: "self-stretch text-[#2A2A2A] font-[Switzer] text-xl lg:text-2xl font-semibold leading-[160%] tracking-[-0.48px]", children: item.title }), _jsx("p", { className: "self-stretch text-[#2A2A2A] font-openSansHebrew text-sm lg:text-base font-normal leading-6 tracking-[-0.16px]", children: item.desc })] }, index))) })] }));
};
