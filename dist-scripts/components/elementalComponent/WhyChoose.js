...
const WhyChoose = () => {
    ...
    const handleColumnEnter = (colIndex) => {
        setHoveredColumnIndex(colIndex);
    };
    const handleColumnLeave = () => {
        setHoveredColumnIndex(null);
    };
    const getColumnBorderClasses = (colIndex, isHeader = false, isLastRow = false) => {
    ...
    const gridColsBase = "grid-cols-[minmax(90px,_1.4fr)_minmax(65px,_1fr)_minmax(65px,_1fr)_minmax(90px,_1.2fr)]";
    const gridColsSm = "sm:grid-cols-[minmax(120px,_1.5fr)_minmax(80px,_1fr)_minmax(80px,_1fr)_minmax(120px,_1.2fr)]";
    const gridColsMd = "md:grid-cols-[minmax(160px,_1.6fr)_1fr_1fr_minmax(160px,_1.3fr)]";
    return (_jsx("div", { className: " text-[#2A2A2A]  font-normal font-[Switzer]  p-5 md:p-12 lg:p-16 xl:p-16 max-w-[96rem]  mx-auto", onMouseLeave: handleColumnLeave, children: _jsxs("div", { className: "mx-auto", children: [_jsx("h2", { className: "text-xl sm:text-2xl md:text-3xl font-semibold text-center text-[#2A2A2A] mb-6 sm:mb-10 md:mb-12 font-[Switzer]", children: "Why Choose Us Over Traditional Hiring or Freelancers?" }), _jsxs("div", { className: "comparison-table-container overflow-x-auto", children: [_jsx("div", { className: `grid ${gridColsBase} ${gridColsSm} ${gridColsMd} text-[11px] sm:text-xs md:text-sm font-medium border-b ${rowBottomBorder}`, children: headers.map((headerText, colIndex) => (_jsx("div", { onMouseEnter: () => handleColumnEnter(colIndex), className: `
                ...
                `, children: headerText }, headerText))) }), comparisonData.map((row, rowIndex) => (_jsx("div", { className: `relative border-b ${rowBottomBorder} last:border-b-transparent`, children: _jsx("div", { className: `grid ${gridColsBase} ${gridColsSm} ${gridColsMd} items-center font-normal font-[Switzer] text-[#292929]   text-xs sm:text-xs md:text-lg`, children: [row.feature, row.inHouse, row.freelancers, row.maxtron].map((cellData, colIndex) => (_jsx("div", { onMouseEnter: () => handleColumnEnter(colIndex), className: `
                        ...
                        ${row.isSpecialFeatureCell && colIndex === 0
                               ...
                               lg:px-[calc(1rem+1px)] lg:py-[calc(1.125rem+1px)] z-10 relative`
                                        : colIndex === 0
                                            ...
                                            : "text-[#292929]"}
                        ${getColumnBorderClasses(colIndex, false, rowIndex === comparisonData.length - 1)}
                      `, children: typeof cellData === "string" ? (cellData) : cellData &&
                                        typeof cellData === "object" &&
                                        "type" in cellData &&
                                        cellData.type === "dot" ? (_jsx(Dot, { colorKey: cellData.colorKey })) : null }, `${row.feature}-${colIndex}`))) }) }, row.feature)))] })] }) }));
};
