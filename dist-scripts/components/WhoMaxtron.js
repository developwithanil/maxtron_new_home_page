...
const WhoMaxtron = () => {
    ...
    return (_jsx("div", { className: "relative w-full bg-white px-5 sm:px-10 md:px-12 lg:px-24 py-16 md:py-24", id: "projects", children: _jsxs("div", { className: "max-w-[1500px] mx-auto", children: [_jsx("header", { className: "mb-4 md:mb-16 lg:mb-24", children: _jsx("h2", { className: "text-[7vw] sm:text-4xl md:text-6xl lg:text-7xl font-light text-[#2A2A2A] font-[IBM Plex Sans] text-left tracking-tighter whitespace-nowrap", children: "Smarter business. Real impact." }) }), _jsxs("div", { className: "grid grid-cols-1 md:grid-cols-[0.42fr_0.58fr] lg:grid-cols-2 gap-10 md:gap-14 lg:gap-32 items-start", children: [_jsx("div", { className: "flex flex-col relative order-2 md:order-1", children: projects.map((project, index) => {
                                const isActive = activeIndex === index;
                                return (_jsx("section", { ref: (el) => {
                                        itemRefs.current[index] = el;
                                    }, className: "min-h-fit md:min-h-[70vh] lg:min-h-[85vh] flex flex-col justify-start md:justify-center py-6 md:py-0 border-t border-gray-100 first:border-none relative", children: _jsxs("div", { className: `transition-all duration-700 ease-out pl-6 md:pl-10 lg:pl-12 relative ${isActive ? "opacity-100" : "opacity-25 translate-x-1"}`, children: [_jsx("div", { className: `absolute left-0 top-0 bottom-0 w-[4px] md:w-[6px] bg-[#7A35C1] rounded-full transition-all duration-700 origin-top
                        ${isActive
                                                    ? "scale-y-100 opacity-100"
                                                    : "scale-y-0 opacity-0"}` }), _jsx("h3", { className: "text-[#111111] font-light text-xl md:text-2xl lg:text-4xl mb-3 md:mb-5 lg:mb-8 leading-tight", children: project.title }), _jsx("p", { className: "text-[#2A2A2A] font-light text-base md:text-lg lg:text-xl leading-relaxed max-w-lg font-[IBM Plex Sans]", children: _jsx(HighlightedText, { text: project.description, serviceKey: project.serviceKey }) }), _jsx("div", { className: "mt-8 md:hidden rounded-2xl overflow-hidden shadow-lg aspect-video", children: _jsx("img", { src: project.image, alt: "", className: "w-full h-full object-cover" }) })] }) }, index));
